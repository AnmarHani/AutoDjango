...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
      ...
      with open(f"{BASE}/{projectName}/settings.py", 'a+') as settingsFile:
        for line in settingsFile:
          lineNum +=1
        settingsFile.seek(lineNum)
        settingsFile.write(f"\nINSTALLED_APPS.append('{appName}')\n")
      with open(f"{BASE}/{projectName}/urls.py", 'a+') as urlsFile:
        for line in urlsFile:
          lineNum +=1
        urlsFile.seek(lineNum)
        urlsFile.write(f"\nurlpatterns.append(path('{appName}/', include('{appName}.urls')))\n")
            
      choice = input("Want to Make models? (y/n): ")
      models = {}
      modelFields = {}
      if(choice == 'y' or choice == 'yes'):
        ...
        choice = ''
      choice = input("Want to Make views [Basic CRUD] for the models? (y/n): ")
      if(choice == 'y' or choice == 'yes'):
...
def Delete{key}(request,id):
    ...
    return Response(True)
        ...
        choice = ''
        choice = input("Want to Make an admin user? (y/n): ")
        if(choice == 'y' or choice == 'yes'):
